...
public class EnvironmentSetup extends AdminBaseClass{
	...
	@Test
	public void AdminBasicSettings ()
	{
		...
		driver.navigate().to(readConfig("baseUrl") + ControlLocators.GRANT_PERMISSIONS_URL);
		WebElement element = utilities.findByXp(driver, ControlLocators.GRANT_ALL_PERMISSIONS_EXPR);
		if(!utilities.contains(element, "selected")) {
			element.click();
		}

		driver.navigate().to(readConfig("baseUrl") + ControlLocators.LIST_SCORE_FACTORS_URL);
		scoreFactor34 = Double.parseDouble(utilities.findByXp(driver, generateXPath_ScoreFactor(34)).getText());
		scoreFactor35 = Double.parseDouble(utilities.findByXp(driver, generateXPath_ScoreFactor(35)).getText());
		...
		Assert.assertEquals(scoreFactor34, SCORE_FACTOR_DEFAULT, "Failed setting 34 Numbered Score Factor");
		Assert.assertEquals(scoreFactor35, SCORE_FACTOR_DEFAULT, "Failed setting 35 Numbered Score Factor");
	}

	public void setScoreFactor(int scoreFactorNumber, double scoreToBeSet)
	{
		driver.navigate().to(readConfig("baseUrl") + "/admin/scorefactor/editScoreFactor.php?sfId=" 
				+ scoreFactorNumber + "&storeId=");
		WebElement EditScoreFactorPoint = utilities.findByName(driver, ControlLocators.EDIT_SCORE_FACTOR_POINTS);
		setText(driver, EditScoreFactorPoint, scoreToBeSet);
		utilities.findListOfElements(driver, "name",ControlLocators.MODIFY_RESULT).get(0).click();
	}

	public void setBrandScoreFactor(int brandScoreFactorNumber)
	{
		driver.navigate().to(readConfig("baseUrl") + ControlLocators.BRAND_SCORE_FACTOR_EXPR + brandScoreFactorNumber);
		WebElement brandScoreFactor_EditPoint = utilities.findByName(driver, ControlLocators.BRAND_SCORE_FACTOR_POINTS);
	...
	{
		String xPath = null;
		xPath = ControlLocators.SCORE_FACTOR_XPATH_EXPR.replaceFirst("sfId=", "sfId="+ scoreFactor);
		return xPath;
	}
